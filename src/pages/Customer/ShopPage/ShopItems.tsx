        ...
        <ModalContent className="max-w-sm">
          ...
          <ModalBody>
            <p className="text-sm text-default-500">
              Unable to load items. Please check your internet connection and
              try again later.
            </p>
          </ModalBody>
          <ModalFooter>
          ...
          {loading &&
            Array.from({ length: 8 }).map((_, index) => (
              <div key={`skeleton-${index}`} className="flex flex-col gap-2">
                ...
                <Skeleton className="h-5 w-full rounded" /> {/* Title */}
                <Skeleton className="h-4 w-3/4 rounded" /> {/* Price Retail */}
                <Skeleton className="h-4 w-1/2 rounded" />{" "}
                {/* Price Wholesale */}
                <Skeleton className="h-12 w-full rounded" /> {/* Description */}
                <Skeleton className="h-4 w-1/3 rounded" /> {/* Stock */}
              </div>
            ))}
        </div>
      </div>
