...
export default function ItemInfoModal({
  ...
  const stringImages =
    item?.item_img?.filter((img): img is string => typeof img === "string") ||
    [];

  const [mainImg, setMainImg] = useState(stringImages[0] || "");
  ...
  useEffect(() => {
    ...
    setPriceVariant(
      hasWholesalePrice && quantity! >= wholesaleMin ? "Wholesale" : "Retail",
    );
          ...
          <>
            <ModalHeader className="flex flex-col gap-2">
              <h2 className="text-lg font-semibold">
                {loading ? "Loading..." : item?.item_title || "Item"}
              </h2>
            </ModalHeader>
