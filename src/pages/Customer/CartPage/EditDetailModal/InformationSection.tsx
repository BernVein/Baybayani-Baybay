    ...
    <>
            ...
            <PopoverContent>
              <div className="px-1 py-2">
                <div className="text-small font-bold">
                  You've selected {selectedItemVariant?.variant_name}
                </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="gap-2 flex flex-row justify-between">
          <span className="text-base text-default-500">Retail Price:</span>
          <span className="text-base text-default-700 font-bold">
            ₱{selectedItemVariant?.variant_price_retail?.toFixed(2) ?? "0.00"}{" "}
            per {item.item_sold_by}
          </span>
        </div>
        <div className="gap-2 flex flex-row justify-between">
          <span className="text-base text-default-500">Wholesale Price:</span>
        ...
        {/* Retail info */}
        <div className="gap-2 flex flex-row justify-between">
          <span className="text-xs text-default-400">Last Price Retail:</span>
          <span className="text-xs text-default-500">
            {selectedItemVariant?.variant_last_updated_price_retail ? (
              <>
                ₱{selectedItemVariant.variant_last_price_retail?.toFixed(2)} (
                {(() => {
                  const updatedAt =
                    selectedItemVariant.variant_last_updated_price_retail;
                  const now = new Date();
              ...
              <span className="italic text-default-400">No change</span>
            )}
          </span>
        </div>
        ...
        <div className="gap-2 flex flex-row justify-between">
          ...
          <span className="text-xs text-default-500">
            {selectedItemVariant?.variant_last_updated_price_wholesale ? (
              <>
                ₱{selectedItemVariant.variant_last_price_wholesale?.toFixed(2)}{" "}
                (
                {(() => {
                  const updatedAt =
                    selectedItemVariant.variant_last_updated_price_wholesale;
      ...
      <div className="flex flex-col gap-2 items-start">
        {canShowWholesale && (
          <div>
            ...
            {quantity >= (selectedItemVariant?.variant_wholesale_item ?? 0) ? (
