...
export function AddOrderModal({
  ...
  const handleClose = async () => {
    ...
    if (cartItems.length > 0) {
      const allIds = cartItems.map((cartItem) => cartItem.cart_item_user_id);
  ...
  const handleCheckout = async (onClose: () => void) => {
    try {
      ...
      await addOrderItems("cb20faec-72c0-4c22-b9d4-4c50bfb9e66f", cartItems);
          ...
          <>
            <ModalHeader className="flex flex-col gap-1">
              <div className="flex flex-col gap-2">
                ...
                <span className="text-sm text-default-500 italic">
                  Changes here are temporary. The order will be officially added
            ...
            <ModalBody className="flex flex-col gap-2">
              <p className="text-sm text-default-500">
                Note: When adding an order, the name will automatically be set
                to{" "}
