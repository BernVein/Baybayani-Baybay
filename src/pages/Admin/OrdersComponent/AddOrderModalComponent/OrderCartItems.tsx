...
export function OrderCartItems({
  ...
  useEffect(() => {
    setSelectedCartItem(items.map((cartItem) => cartItem.cart_item_user_id));
  }, [items]);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("baybayani:cart-updated", handleCartUpdate);
  }, [refetch]);
        ...
        <div className="flex flex-col gap-3 mt-4">
          <div className="flex justify-between items-center px-1">
            <div className="flex flex-col items-start gap-1">
              <span className="text-sm font-semibold text-default-700">
                Selected Items ({selectedCartItem.length}/{items.length})
              </span>
            ...
            <Button
              color="danger"
              isDisabled={selectedCartItem.length === 0 || isDeleting}
          ...
          <div className="max-h-[400px] overflow-y-auto pr-2 flex flex-col gap-3">
            <CheckboxGroup
              ...
              onValueChange={(value) => setSelectedCartItem(value as string[])}
            >
              {items.map((item) => (
                <CartItem
