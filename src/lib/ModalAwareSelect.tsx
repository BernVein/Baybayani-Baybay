...
function ModalAwareSelectInner<T extends object>(
  ...
  const computeContainer = useCallback(() => {
    ...
    const wrapper = triggerRef.current.closest('[data-slot="wrapper"]') as HTMLElement | null
  ...
  const mergedRef = useMemo(() => mergeRefs(forwardedRef, triggerRef), [forwardedRef])
...
export const ModalAwareSelect = forwardRef(ModalAwareSelectInner) as <T extends object>(
  props: ModalAwareSelectProps<T> & { ref?: React.ForwardedRef<HTMLSelectElement> },
) => JSX.Element
