...
export const useFetchProductsUI = (
	...
	const fetchItems = useCallback(
		async (showLoading: boolean = true) => {
			...
			try {
				...
				const formatted: ItemTableRow[] = (data ?? []).map(
					(item: any) => {
						...
						return {
							...
							variant_stock: variantStocks.reduce(
								(sum: number, v: any) =>
									sum + (v.currentStock ?? 0),
								0,
							),
							item_img_url:
								item.Item_Image?.[0]?.item_image_url ?? "",
							item_category:
	...
	useEffect(() => {
		...
		const channel = supabase
			.channel(`admin-products-realtime-${channelId}`)
			.on(
				"postgres_changes",
